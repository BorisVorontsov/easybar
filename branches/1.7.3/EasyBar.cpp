...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, LPWSTR lpCmdLine, int nShowCmd)
{
	...
	__try
	{
		...
		ReadFileVersion(lpAppPath, lpAppVersionMM, sizeof(lpAppVersionMM), RFV_MAJOR | RFV_MINOR);
		swprintf(lpStdWndTitle, L"%s %s", APP_NAME, lpAppVersionMM);
...
{
	WCHAR lpExt[64] = {};
	LPWSTR *lpCmdLineArgs = 0;
	ULONG i = 0, lArgsCnt = 0, lFileCnt = 0;
	lpCmdLineArgs = CommandLineToArgvW(lpCmdLine, (int *)&lArgsCnt);
	if (lpCmdLineArgs)
	{
		for (i = 0; i < lArgsCnt; i++)
		{
			if (i == 0)
			{
				if (!*pAdd)
				{
					if (wcscmp(lpCmdLineArgs[i], APP_CL_KEY_ADD) != 0)
					{
			...
			SP_ExtractRightPart(lpCmdLineArgs[i], lpExt, '.');
			if (SFT_IsMemberOfCategory(lpExt, SFTC_PLAYLIST))
			{
				lFileCnt += LoadPlaylist(lpCmdLineArgs[i]);
			}
			else if (IsDirectory(lpCmdLineArgs[i]))
			{
				lFileCnt += LoadDirectory(lpCmdLineArgs[i]);
			}
			else
			{
				pFileCollection->AppendFile(lpCmdLineArgs[i]);
				lFileCnt++;
			}
		}
		for (i = 0; i < (ULONG)lArgsCnt; i++)
			LocalFree(lpCmdLineArgs[i]);
