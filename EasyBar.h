...
//Unicode-константы
#define APP_DEV_STAGE				L"\0" //Alpha (n), Beta (n), RC (n), '\0' (release)

#define APP_NAME					L"EasyBar"
#define APP_COPYRIGHT				L"Copyright © Boris Vorontsov & Co., 2007 - 2010"
...
__inline SFTCATEGORY SFT_IsCategory(LPCWSTR lpItem)
{
	if (_wcsicmp(lpItem, L"[VIDEO]") == 0) return SFTC_VIDEO;
	if (_wcsicmp(lpItem, L"[AUDIO]") == 0) return SFTC_AUDIO;
	if (_wcsicmp(lpItem, L"[MIDI]") == 0) return SFTC_MIDI;
	if (_wcsicmp(lpItem, L"[IMAGE]") == 0) return SFTC_IMAGE;
	if (_wcsicmp(lpItem, L"[PLAYLIST]") == 0) return SFTC_PLAYLIST;
	if (_wcsicmp(lpItem, L"[OTHER]") == 0) return SFTC_OTHER;
	if (_wcsicmp(lpItem, L"[RESERVED]") == 0) return SFTC_RESERVED;
	return SFTC_NONE;
}

__inline BOOL SFT_IsMemberOfCategory(LPCWSTR lpItem, SFTCATEGORY cCategory)
{
	for (ULONG i = 0; i < APP_SFT_UBOUND; i++)
	{
		if (SFT_IsCategory(APP_SFT[i]) == cCategory)
		{
			...
			{
				if (_wcsicmp(lpItem, APP_SFT[j]) == 0) return TRUE;
			}
			break;
		}
	}
...
void CloseTrack();
DWORD LoadDirectory(LPCWSTR lpPath, BOOL bIncImages = FALSE);
void InitMainWnd(BOOL bCreate = TRUE);
void PPSetDefFileInfo(HWND hPPWnd, LPWSTR lpFileName);
void UpdateCFTitle(LPCWSTR lpFileName, BOOL bMainWndTitleOnly = FALSE);
...
extern LPWSTR lpRecentDir;
extern LPWSTR lpRecentURL;
...
extern LPWSTR lpExternalFilters[APP_MAX_STRINGS];
