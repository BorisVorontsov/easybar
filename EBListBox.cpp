...
BOOL CEBListBox::InitEBListBox(HWND hListBox)
{
	if (hListBox)
	{
		if ((GetWindowLongPtr(hListBox, GWL_STYLE) & LBS_OWNERDRAWFIXED) != LBS_OWNERDRAWFIXED)
			return FALSE;
		int intItemsCnt = (int)SendMessage(hListBox, LB_GETCOUNT, 0, 0);
		...
		m_lOldLBProc = SetWindowLongPtr(hListBox, GWLP_WNDPROC, (LONG_PTR)ListBoxProc);
		m_hListBox = hListBox;
		Refresh();
	}
	else
	{
		...
		int intItemsCnt = (int)SendMessage(m_hListBox, LB_GETCOUNT, 0, 0);
		if (intItemsCnt != LB_ERR)
		{
			LPLBITEMDESC pLBID;
			for (UINT i = 0; i < (UINT)intItemsCnt; i++)
		...
		SetWindowLongPtr(m_hListBox, GWLP_WNDPROC, m_lOldLBProc);
		m_lOldLBProc = 0;
...
LRESULT CALLBACK CEBListBox::ListBoxProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		case WM_ERASEBKGND:
		{
			...
			intItemsCnt = (int)SendMessage(hWnd, LB_GETCOUNT, 0, 0);
		...
		case WM_PAINT:
		{
			int intItemsCnt = (int)SendMessage(hWnd, LB_GETCOUNT, 0, 0);
			if ((intItemsCnt == LB_ERR) || !intItemsCnt)
			{
				PAINTSTRUCT PS = {};
...
DWORD CEBListBox::DrawItem(WPARAM wParam, LPARAM lParam)
{
	...
	LPLBITEMDESC pLBID;
	TCHAR lpText[MAX_PATH + 64] = {};
	RECT RCP = {}, RCT = {};
	BITMAP BMLP = {}, BMNI = {}, BMSI = {};
	HDC hTmpDC;
...
BOOL CEBListBox::Sort(BOOL bReverse)
{
	...
	UINT i, j;
	SIZE_T szTextSize;
	LPWSTR *lpItems = new LPWSTR[intItemsCnt];
	...
	{
		szTextSize = (SIZE_T)SendMessage(m_hListBox, LB_GETTEXTLEN, i, 0);
		lpItems[i] = new WCHAR[szTextSize + 1];
		ZeroMemory(lpItems[i], (szTextSize + 1) * sizeof(WCHAR));
		SendMessage(m_hListBox, LB_GETTEXT, i, (LPARAM)lpItems[i]);
		pIDArr[i] = new ItemData;
		pIDArr[i]->pLBID = NULL;
		pIDArr[i]->lpItem = lpItems[i];
		pLBID = (LPLBITEMDESC)SendMessage(m_hListBox, LB_GETITEMDATA, i, 0);
		if (((int)pLBID != LB_ERR) && pLBID)
		{
	...
	qsort(lpItems, intItemsCnt, sizeof(LPWSTR), (bReverse)?CompareReverse:Compare);
	
	for (i = 0; i < (UINT)intItemsCnt; i++)
	{
		SendMessage(m_hListBox, LB_ADDSTRING, 0, (LPARAM)lpItems[i]);
		for (j = 0; j < (UINT)intItemsCnt; j++)
		{
			if (!pIDArr[j] || !pIDArr[j]->pLBID) continue;
			if (lpItems[i] == pIDArr[j]->lpItem)
			{
					  ...
					  BOOL bDisabled, BOOL bSelected, BOOL bHighlighted)
{
	...
	{
		HFONT hOldFont;
		TCHAR lpCol1[MAX_PATH] = {}, lpCol2[64] = {};
